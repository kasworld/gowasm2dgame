...
loop:
	for {
		...
		case pk := <-wsc.sendCh:
			oldbuf := bufPool.Get()
			sendBuffer, err := w2d_packet.Packet2Bytes(&pk, wsc.marshalBodyFn, oldbuf)
			if err != nil {
				bufPool.Put(oldbuf)
				break loop
			}
...
func (wsc *Connection) handleWebsocketMessage(this js.Value, args []js.Value) interface{} {
	...
	aBuff.Call("then",
		js.FuncOf(func(this js.Value, args []js.Value) interface{} {
			...
			rPk := w2d_packet.NewRecvPacketBufferByData(rdata)
...
func (wsc *Connection) EnqueueSendPacket(pk w2d_packet.Packet) error {
	select {
	case wsc.sendCh <- pk:
		return nil
	default:
